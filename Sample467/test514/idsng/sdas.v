...
// created on        : 2019-12-02T12:01:56.326+05:30
// created by        :
...
module sdas_ids #(
    ...
    parameter sdas_address_width = addr_width
    )
    ...
    input  [31 : 0] reg_name_dfs_in,    // FIELD : DFS
    
    
    // HW WRITE-ABLE SIGNAL FOR EACH FIELD
    input   reg_name_dfs_in_enb,    // FIELD : DFS
    
    
    // READ DATA SIGNAL FOR EACH FIELD
    output [31 : 0] reg_name_dfs_r,    // FIELD : DFS
    ...
    reg [31 : 0] reg_name_dfs_q ;      // FIELD : DFS
    ...
    //
    // FIELDS   :
    //     31:0 : dfs  ( SW : Read-Write HW : Read-Write )
    ...
    assign reg_name_decode  = (address[sdas_address_width-1 : 0]    == reg_name_offset[sdas_address_width-1 : 0] ) ? 1'b1 : 1'b0;
    ...
    always @(posedge clk)
        ...
        if (!reset_l)
            begin
                ...
                reg_name_dfs_q  <= 32'd0;
            end
        else
            begin
            if (reg_name_dfs_in_enb)   // DFS : HW Write
                begin
                    reg_name_dfs_q <= reg_name_dfs_in;
                end
            else
                begin
                    
                if (reg_name_wr_valid)   // DFS : SW Write
                    begin
                        reg_name_dfs_q <=  ( wr_data[31 : 0] & reg_enb[31 : 0] ) | (reg_name_dfs_q & (~reg_enb[31 : 0]));
                    end
    ...
    assign reg_name_dfs_r  =  reg_name_dfs_q ;    // Field : DFS
    
    
    assign reg_name_rd_data = reg_name_decode ? {reg_name_dfs_q} : 32'b00000000000000000000000000000000;
