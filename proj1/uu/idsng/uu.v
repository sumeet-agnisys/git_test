...
// created on        : 2019-12-24T12:26:02.856+05:30
// created by        :
// generated by      : Sameer
...
module uu_ids #(
    ...
    parameter reg2_count                              = 10,
    
    parameter reg2_address_width                      = addr_width,
    ...
    output  [reg2_count-1 : 0] reg2_enb,
    
    // HW WRITE DATA SIGNAL FOR EACH FIELD
    input  [reg2_count*32-1 : 0] reg2_f2_in,    // FIELD : F2
    
    
    // HW WRITE-ABLE SIGNAL FOR EACH FIELD
    input  [reg2_count-1 : 0] reg2_f2_in_enb,    // FIELD : F2
    
    
    // READ DATA SIGNAL FOR EACH FIELD
    output [reg2_count*32-1 : 0] reg2_f2_r,    // FIELD : F2
    ...
    wire    reg2_decode[reg2_count-1 : 0];         // DECODE
    wire    [reg2_count-1 : 0]reg2_wr_valid;       // WRITE VALID
    wire    [bus_width-1 : 0] reg2_rd_data[reg2_count-1 : 0];          // READ DATA
    wire    [64-1 : 0] reg2_offset[reg2_count-1 : 0];  // OFFSET
    wor   [bus_width-1 : 0] reg2_rd_data_wor;
    
    // BUFFER SIGNAL FOR EACH FIELD
    reg [31 : 0] reg2_f2_q[reg2_count-1 : 0] ;      // FIELD : F2
    ...
    assign reg3_offset = block_offset+'h2C;
    ...
    generate
        genvar reg2_i;
        for( reg2_i = 0; reg2_i < reg2_count; reg2_i = reg2_i + 1)
        begin : reg2_gen
            
            
            //----------------------------------------------------------------------------------------------------
            // REGISTER      :  REG2
            // ADDRESS       :  block_offset+'h4 + reg2_i * 'h4                WIDTH : 32
            // HW ACCESS     :  READ-WRITE
            // SW ACCESS     :  READ-WRITE
            //
            // FIELDS   :
            ...
            assign reg2_wr_valid[reg2_i] = reg2_decode[reg2_i] && wr_stb;
            assign reg2_enb[reg2_i]      = reg2_wr_valid[reg2_i];
            assign reg2_offset[reg2_i] = block_offset+'h4 + reg2_i * 'h4;
            assign reg2_decode[reg2_i]  = (address[reg2_address_width-1 : 0]    == reg2_offset[reg2_i][reg2_address_width-1 : 0] ) ? 1'b1 : 1'b0;
            ...
            always @(posedge clk)
                ...
                if (!reset_l)
                    begin
                        ...
                        reg2_f2_q[reg2_i]  <= 32'd0;
                    end
                else
                    begin
                    if (reg2_f2_in_enb[reg2_i])   // F2 : HW Write
                        begin
                            reg2_f2_q[reg2_i] <= reg2_f2_in[(reg2_i) *32+31 :  (reg2_i) *32 ];
                        end
                    else
                        begin
                            
                        if (reg2_wr_valid[reg2_i])   // F2 : SW Write
                            begin
                                reg2_f2_q[reg2_i] <=  ( wr_data[31 : 0] & reg_enb[31 : 0] ) | (reg2_f2_q[reg2_i] & (~reg_enb[31 : 0]));
                            end
            ...
            // ===================================================
            // HW OUTPUT READ DATA FOR EACH FIELD
            assign reg2_f2_r[(reg2_i) *32+31 :  (reg2_i) *32 ]  =  reg2_f2_q[reg2_i] ;    // Field : F2
            
            
            assign reg2_rd_data[reg2_i] = reg2_decode[reg2_i] ? {reg2_f2_q[reg2_i]} : 32'b00000000000000000000000000000000;
            assign reg2_rd_data_wor = reg2_rd_data[reg2_i];
