...
// generated from    : D:\AgnisysProjects\git_test\Proj1\test\test31.idsng
...
module test31_ids(
    ...
    parameter test31_address_width = addr_width;
    //-----------------------------------------------------
    ...
    input   reg_name_f3_in_enb ;      // FIELD : F3
    
    // BUFFER SIGNAL FOR EACH FIELD
    reg  [7 : 0] reg_name_f1_q ;      // FIELD : F1
    reg          reg_name_f2_q ;      // FIELD : F2
    reg  [3 : 0] reg_name_f3_q ;      // FIELD : F3
    
    // READ DATA SIGNAL FOR EACH FIELD
    output   [7 : 0] reg_name_f1_r ;      // FIELD : F1
    output           reg_name_f2_r ;      // FIELD : F2
    output   [3 : 0] reg_name_f3_r ;      // FIELD : F3
    
    // HW WRITE DATA SIGNAL FOR EACH FIELD
    input    [3 : 0] reg_name_f3_in ;      // FIELD : F3
    ...
    //
    // FIELDS   :
    //      0:7 : f1  ( SW : Read-Write HW : Read-only )
    //        9 : f2  ( SW : Read-Write HW : Read-only )
    //    13:10 : f3  ( SW : Read-Write HW : Read-Write )
    ...
    assign reg_name_decode  = (address[test31_address_width-1 : 0]    == reg_name_offset[test31_address_width-1 : 0] ) ? 1'b1 : 1'b0;
    ...
    always @(posedge clk)
        ...
        if (!reset_l)
            ...
            if (reg_name_wr_valid)   // F1 : SW Write
                begin
                    reg_name_f1_q <=  ( wr_data[7 : 0] & reg_enb[7 : 0] ) | (reg_name_f1_q & (~reg_enb[7 : 0]));
                end
    ...
    always @(posedge clk)
        ...
        if (!reset_l)
            ...
            if (reg_name_wr_valid)   // F2 : SW Write
                begin
                    reg_name_f2_q <=  ( wr_data[9] & reg_enb[9] ) | (reg_name_f2_q & (~reg_enb[9]));
                end
    ...
    always @(posedge clk)
        ...
        if (!reset_l)
            ...
            begin
            if (reg_name_f3_in_enb)   // F3 : HW Write
                begin
                    reg_name_f3_q <= reg_name_f3_in;
                end
            else
                begin
                ...
                if (reg_name_wr_valid)   // F3 : SW Write
                    begin
                        reg_name_f3_q <=  ( wr_data[13 : 10] & reg_enb[13 : 10] ) | (reg_name_f3_q & (~reg_enb[13 : 10]));
                    end
    ...
    assign reg_name_f1_r  =  reg_name_f1_q ;    // Field : F1
    assign reg_name_f2_r  =  reg_name_f2_q ;    // Field : F2
    assign reg_name_f3_r  =  reg_name_f3_q ;    // Field : F3
    
    
    assign reg_name_rd_data = reg_name_decode ? {18'b0 ,reg_name_f3_q , reg_name_f2_q , 1'b0 ,reg_name_f1_q} : 32'b00000000000000000000000000000000;
