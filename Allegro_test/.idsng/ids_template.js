...
function addIntoHistory() {
    if (isRegViewOn()) {
        if (regHistoryArraay[regHistoryIndex - 1] !== document.getElementById(REG_DIV_CONTAINER).innerHTML) {
            var divhist = document.getElementById(REG_DIV_CONTAINER).innerHTML;
            regHistoryArraay[regHistoryIndex] = divhist;
            regHistoryIndex++;
        }
    } else if (isParamViewOn()) {
        if (paramHistoryArr[paramHistoryIndex - 1] !== document.getElementById(PARAM_DIV_CONTAINER).innerHTML) {
            paramHistoryArr[paramHistoryIndex] = document.getElementById(PARAM_DIV_CONTAINER).innerHTML;
            paramHistoryIndex++;
            //localStorage.setItem(getParamHistoryID(), JSON.stringify(paramHistoryArr));
        }
    } else if (isSeqViewOn()) {
        if (seqHistoryArr[seqHistoryIndex - 1] !== document.getElementById(SEQ_DIV_CONTAINER).innerHTML) {
            ...
            seqHistoryIndex++;
            //localStorage.setItem(getSeqHistoryID(), JSON.stringify(seqHistoryArr));
        }
    } else if (isSpreadViewOn()) {
        if (spreadHistoryArr[spreadHistoryIndex - 1] !== document.getElementById(SPREAD_DIV_CONTAINER).innerHTML) {
            spreadHistoryArr[spreadHistoryIndex] = document.getElementById(SPREAD_DIV_CONTAINER).innerHTML;
            spreadHistoryIndex++;
            //localStorage.setItem(getSpreadHistoryID(), JSON.stringify(spreadHistoryArr));
        }
    }
...
function getHistory() {
    ...
    try {
        ...
        if (document.getElementById(DIV_HISTORY) !== null) {
            document.getElementById(DIV_HISTORY).remove();//.innerHTML = "";
        }

    } catch (e) {
        alert("err getHistory : " + e.message);
    }
}
...
$(document).ready(function () {
    ...
    /*
     $(document).on('focusin', '.step', function () {
     var isobfound = false;
     for (var ob in DESC_CLS_OBJ_LIST) {
     if (DESC_CLS_OBJ_LIST[ob] === this) {
     isobfound = true;
     ...
     if (!isobfound) {
     DESC_CLS_OBJ_LIST.push(this);
     var arr=create_reg_arr(document.getElementById("regdivcontainer"));
    ...
    $(document).on("mouseover", ".paramname", function () {
    ...
    $(document).on("mouseout", ".paramname", function () {
...
/*
 ...
 addcontextmenuonparamreg();
 }
 
 function addcontextmenuonparamreg() {
 var reglst = document.getElementById("paramcontainer").getElementsByClassName("paramname");
 for (var i = 0; i < reglst.length; i++)
 {
 reglst[i].setAttribute('oncontextmenu', 'openparamregcontextmenu(event);');
 }
 }
 */
...
function scrollFieldCells(e, isdown) {
    try {
        ...
        if (autolist !== null && autolist.style.display !== "none") {
        ...
        if (isdown) {
            tab.rows[row.rowIndex + 1].cells[cellindex - 1].focus();
        } else {
            tab.rows[row.rowIndex - 1].cells[cellindex + 1].focus();
        }
